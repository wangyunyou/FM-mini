...
- **样式是中性实现，没有设计稿依据**。配色/间距按常见小程序布局定的，接入 Figma 后整体替换即可：
  样式侧改 `src/app.scss` 的 `page` 块，配置侧（导航栏/tabBar 不支持 CSS 变量）改 `src/constants/theme.ts`，两处必须同值。
  当前色板的定色依据与踩坑清单见 [AGENTS.md](./AGENTS.md) 的「样式」节。
